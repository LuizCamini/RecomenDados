...
OR
ALTER VIEW
        VW_SELECTEXTRATONAOCONCILIADO AS
SELECT
        DATALANCTO  ,
        NRCHEQUE    ,
        NRORDEM     ,
...
FROM
        (
                ...
                INNER JOIN
                        FORNECEDORES F
                ON
                        F.ID = CP.FORNECEDORID
                INNER JOIN
                        PlanoContas AS PCC
                ON
                        CP.CENTROCUSTOID = PCC.CentroCustoId
                INNER JOIN
                        PlanoContas AS PCS
                ON
                        CP.SUBCUSTOID = PCS.SubcustoId
                INNER JOIN
                        CentrosCustos AS CC
                ON
                        CC.Id = PCC.CentroCustoId
                INNER JOIN
                        Subcustos AS SC
                ON
                        SC.ID = PCS.SubcustoId
                WHERE
                        (
                                CP.CONCILIADO       = 0
                        OR      CP.CONCILIADO IS NULL)
                AND     CP.DATAPGTO IS NOT NULL
                ...
                SELECT
                        ...
                        SC.DESCRICAO                                              AS SUBCUSTO
                FROM
                        LANCAMENTOSTRANSFERENCIAS LT
                INNER JOIN
                        SITRAWEBNEW.DBO.FILIAIS FIL
                ON
                        FIL.FILIALID = LT.FILIALID
                INNER JOIN
                        PlanoContas AS PCC
                ON
                        LT.CENTROCUSTOID = PCC.CentroCustoId
                INNER JOIN
                        PlanoContas AS PCS
                ON
                        LT.SUBCUSTOID = PCS.SubcustoId
                INNER JOIN
                        CentrosCustos AS CC
                ON
                        CC.Id = PCC.CentroCustoId
                ...
                FROM
                        (
                                ...
                                FROM
                                        RECEBIMENTO R
                                INNER JOIN
                                        RECEBIMENTOPAGAMENTOS RP
                                ON
                                        RP.REBID           = R.REBID
                                AND     RP.TIPOPAGAMENTOID = 1
                                WHERE
                                        R.STATUSRECEBIMENTOID           = 2
                                AND     RP.DATACHEQUEDEPOSITO IS NOT NULL ) REB
                GROUP BY
                        DATALANCTO  ,
                        NRCHEQUE    ,
                        NRORDEM     ,
                ...
                INNER JOIN
                        CLIENTES CLI
                ON
                        CLI.CLIENTEID = R.CLIENTEPAGADORID
                WHERE
                ...
                INNER JOIN
                        CLIENTES CLI
                ON
                        CLI.CLIENTEID = FV.TOMADORID
                INNER JOIN
                        SITRAWEBNEW.DBO.FILIAIS FIL
                ON
                        FIL.FILIALID = FV.FILIALORIGEMID
                WHERE
                        FV.STATUSFATURAMENTOID     = 3
                AND     FV.DATAPAGAMENTO IS NOT NULL ) MAIN 
GO 
CREATE
OR
ALTER VIEW
        VW_SELECTEXTRATOCONCILIADO AS
SELECT
        DATALANCTO  ,
        NRCHEQUE    ,
        NRORDEM     ,
